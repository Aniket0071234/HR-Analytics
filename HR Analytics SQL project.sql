...
from hr__1 inner join hr__2
on hr__1.EmployeeNumber = hr__2.`Employee ID`
group by hr__1.Department;

#KPI 5:- Job Role VS Work Life Balance
select hr__1.JobRole,
format(avg(hr__2.WorkLifeBalance),2) as Average_WorkLifeBalance
from hr__1 
inner join hr__2 on hr__2.`Employee ID` = hr__1.EmployeeNumber
...
(COUNT(hr__1.attrition) / (SELECT COUNT(*) FROM hr__1 WHERE attrition = 'Yes')) * 100 AS 'attrition_rate'
FROM hr__1
